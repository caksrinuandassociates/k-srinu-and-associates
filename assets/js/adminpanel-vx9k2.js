...
async function saveSettings(e){ e.preventDefault(); const btn=refs.settingsForm?.querySelector("button[type='submit']"); busy(btn,true,"Saving..."); try{ const id=$("s-id").value; const payload={phone:$("s-phone").value,email:$("s-email").value,whatsapp:$("s-whatsapp").value,address:$("s-address").value,map_link:$("s-map").value,footer_text:$("s-footer").value}; let savedRow=null; if(id){ const {data:updateData,error:updateError}=await supabaseClient.from("site_settings").update(payload).eq("id",id).select().single(); if(updateError && updateError.code !== "PGRST116") throw updateError; if(updateData) savedRow=updateData; } if(!savedRow){ const {data:insertData,error:insertError}=await supabaseClient.from("site_settings").insert([payload]).select().single(); if(insertError) throw insertError; savedRow=insertData||null; } if(!savedRow) throw new Error("Site settings save failed: no row returned."); setMsg(refs.adminMsg,"Site settings saved."); await loadSettings(); }catch(err){ setMsg(refs.adminMsg,err.message,true);} finally{ busy(btn,false);} }
